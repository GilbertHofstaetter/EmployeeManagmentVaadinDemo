...
public class DocumentForm extends VerticalLayout implements ComponentEventListener<AllFinishedEvent> {
    ...
    void dataToUI(String filterText) {
        ...
        documentGrid.setItems(getDocumentDataProvider(company.getId(), "%" + filterText + "%"));
    }
        ...
        DataProvider<Document, Void> dataProvider =
                DataProvider.fromCallbacks(
                        ...
                        query -> {
                            ...
                            Page<Document> documents = documentRepository.findByCompanyIdAndFileNameLike(id, filterText, sorted);

                            return documents.stream();
                        },
                        // Second callback fetches the total number of items currently in the Grid.
                        // The grid can then use it to properly adjust the scrollbars.
                        query -> documentRepository.countByCompanyIdAndFileNameLike(id, filterText));
