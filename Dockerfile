...
WORKDIR /app
COPY . /app
RUN rm -r /app/node_modules #has to be removed to prevent build failures -> only if present through previous local build
RUN rm -r /app/frontend #has to be removed to prevent build failures -> only if present through previous local build
